...
DBus *DB_Hardware = NULL;

DBus *DBus_Use()
{
    if (DB_Hardware == NULL)
    {
        DB_Hardware = new DBus();
    }
...
void DBus::unpack()
{
    if (buff_[0] == 0 && buff_[1] == 0 && buff_[2] == 0 && buff_[3] == 0 && buff_[4] == 0 && buff_[5] == 0)
        return;
...
void DBus::getData()
{
    if ((is_success) && (is_update_))
    {
        // d_bus_data.ch_r_x = static_cast<double>(d_bus_data_.ch0 );
        // d_bus_data.ch_r_y = static_cast<double>(d_bus_data_.ch1 );
        // d_bus_data.ch_l_x = static_cast<double>(d_bus_data_.ch2 / 660.0);
        // d_bus_data.ch_l_y = static_cast<double>(d_bus_data_.ch3 / 660.0);
        d_bus_data.ch_r_x = static_cast<double>(d_bus_data_.ch0 / 1); //左右   <-660  660>
        d_bus_data.ch_r_y = static_cast<double>(d_bus_data_.ch1 / 1);
        d_bus_data.ch_l_x = static_cast<double>(d_bus_data_.ch2 / 1);
        d_bus_data.ch_l_y = static_cast<double>(d_bus_data_.ch3 / 1);
        ...
        d_bus_data.s_l = d_bus_data_.s1;   // 上中下 1 
        ...
        // ROS_WARN("rc_sw: %d",d_bus_data.s_r);
        // ROS_WARN("rc_ch0: %f",d_bus_data.ch_r_x);
    }
}
